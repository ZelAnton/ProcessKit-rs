...
runner keeps compiling: `start` is defaulted to `ErrorKind::Unsupported`.
...
- **`Reply::ok(stdout)`** — exit 0. **`Reply::fail(code, stderr)`** — non-zero
  ...
  **`Reply::timeout()`** — a timed-out run (the checking helpers raise
  `ErrorKind::Timeout` from it, carrying the command's own configured deadline). On a
...
- **`Reply::pending()`** — parks the call until the
  ...
  fires, resolving with `ErrorKind::Cancelled` — so a test can prove an
...
- **No match and no fallback is a loud error** (`ErrorKind::Spawn`, not-found) —
...
| Miss | strict `ErrorKind::CassetteMiss` (distinct from a missing program — `is_not_found()` is `false`) — replay never spawns a surprise subprocess; a stale cassette fails loudly |
| Timeouts | a recorded timed-out run replays as one, surfacing `ErrorKind::Timeout` with the *replaying* command's deadline |
| Format | pretty-printed JSON with a `version` field; unknown versions / corrupt files / an entry with a contradictory outcome / a file over 64 MiB are `ErrorKind::Io(InvalidData)`, a missing file keeps `NotFound` |
...
impl<R: ProcessRunner> Git<R> {
    ...
    /// `Result`, typically an `ErrorKind::Parse` naming the program.
...
(infallible) and `try_parse` (fallible → `ErrorKind::Parse`).
