...
pub(crate) struct SharedLines {
    ...
    /// [`ErrorKind::Io`](crate::ErrorKind::Io) instead of a silent short read reported as
...
struct Inner {
    ...
    /// consuming path turns this into [`ErrorKind::OutputTooLarge`](crate::ErrorKind::OutputTooLarge).
    overflowed: bool,
...
impl SharedLines {
        ...
        let mut policy_dropped = false;
        {
            ...
            if inner.discarding {
                ...
                match inner.mode {
                    ...
                    // turns `overflowed` into `ErrorKind::OutputTooLarge`.
                    OverflowMode::Error => {
    ...
    /// [`ErrorKind::Io`](crate::ErrorKind::Io). Only the *first* error is kept (a later
    ...
    /// [`ErrorKind::Io`](crate::ErrorKind::Io); `None` means the stream drained to a clean
...
/// surface the incomplete capture as [`ErrorKind::Io`](crate::ErrorKind::Io) rather than
    ...
    loop {
        ...
        if eof {
            ...
            if let Some(e) = read_err {
                ...
                // incomplete capture as `ErrorKind::Io`.
...
mod tests {
    ...
    async fn clean_eof_records_no_read_error() {
        ...
        // `ErrorKind::Io` — the non-regression guard against a false-positive read error.
        let sink = SharedLines::new(&OutputBufferPolicy::unbounded());
        pump_lines(&b"a\nb\n"[..], encoding_rs::UTF_8, None, sink.clone()).await;
        assert_eq!(sink.drain(), vec!["a", "b"]);
        assert!(
