...
///
/// Spawn failures, a non-zero exit ([`ErrorKind::Exit`]), timeouts, and IO
/// errors fold into one structured kind, so callers can pattern-match on the
/// failure mode — via [`kind`](Self::kind) — instead of parsing strings.
///
/// A thin wrapper: `Error` boxes its [`ErrorKind`] (`Box<ErrorKind>`) so the
/// `Ok` path of every `Result<T, Error>` stays small — the largest kind
/// ([`Exit`](ErrorKind::Exit) / [`Timeout`](ErrorKind::Timeout) /
/// [`Signalled`](ErrorKind::Signalled), each carrying two captured streams)
/// no longer inflates every fallible return in the crate.
///
/// `Display`, `Debug`, and [`std::error::Error::source`] all delegate to the
/// boxed [`ErrorKind`] — see there for the "manual, not derived `Debug`"
/// rationale (the stream-carrying kinds bound their captured output to a
/// 200-byte preview rather than dumping it into a `{e:?}` log line or
/// `.unwrap()` panic message).
pub struct Error {
    kind: Box<ErrorKind>,
}

/// The kinds of failure [`Error`] can carry — reach one via [`Error::kind`].
///
/// `Debug` is **manual, not derived**: the [`Exit`](Self::Exit) variant
...
pub enum ErrorKind {
    ...
    ///
    /// Distinct from [`Spawn`](Self::Spawn), which covers OS-level failures
    ...
    /// [`diagnostic`](Error::diagnostic). Consumers also classify on these fields
    ...
    /// [`diagnostic`](Error::diagnostic), capped at 200 bytes — `` `git` exited
    ...
    /// reachable via [`diagnostic`](Error::diagnostic) and the public fields
    /// rather than lost. Empty when the producing path captured nothing (a
    /// streaming probe such as `first_line`, which never buffers).
    ///
    /// The one-line `Display` message appends the **last non-empty line** of
    /// [`diagnostic`](Error::diagnostic), capped at 200 bytes — just like
    /// [`Exit`](Self::Exit) — so a log line stays actionable without dumping
    ...
    Timeout {
        ...
        /// path captured raw bytes — see [`Exit`](Self::Exit)'s `stdout_bytes`
    ...
    /// becoming ready.
    ///
    /// Distinct from [`Timeout`](Self::Timeout): a probe deadline is separate
    ...
    /// full, so — like the [`Exit`](Self::Exit) streams — both `Display` and
    ...
    /// the [`limit_kind`](Error::limit_kind) / [`limit_reason`](Error::limit_reason)
    /// accessors.
    #[cfg(feature = "limits")]
    ...
    ResourceLimit {
        ...
        /// fields (see [`Parse`](Self::Parse)'s `message`) in `Display`/`Debug`.
        detail: String,
    },
    ...
    /// tree was killed.
    ///
    /// Asymmetric with [`Timeout`](Self::Timeout) by design: a timeout is
    ...
    /// checked first).
    ///
    /// Unlike [`Timeout`](Self::Timeout) / [`Signalled`](Self::Signalled),
    ...
    /// be noise. [`diagnostic`](Error::diagnostic) returns `None`.
    ...
    /// else `None`. On **Windows** a killed process reports [`Exit`](Self::Exit)
    ...
    /// Unix.
    ///
    /// Distinct from [`Exit`](Self::Exit): a signal-terminated run has no exit
    ...
    /// [`diagnostic`](Error::diagnostic) and the public fields. The one-line
    /// `Display` appends the bounded diagnostic tail, like [`Exit`](Self::Exit).
    ...
    /// run otherwise succeeded** — a non-zero [`Exit`](Self::Exit), a
    /// [`Signalled`](Self::Signalled), or a [`Timeout`](Self::Timeout) is the
    ...
    /// [`Spawn`](Self::Spawn) / [`NotFound`](Self::NotFound)).
    ///
...
impl Error {
    /// The [`ErrorKind`] this error carries
    pub fn kind(&self) -> &ErrorKind {
        &self.kind
    }

    /// Unwraps this error into its owned [`ErrorKind`].
    ...
    /// non-zero [`Exit`](ErrorKind::Exit), a [`Timeout`](ErrorKind::Timeout) (the
    /// partial output of a hung-then-killed tool), and a
    /// [`Signalled`](ErrorKind::Signalled) crash. Returns `None` when there is no
    /// captured output to show — a silent run (both streams blank) or a variant
    /// that carries none ([`Spawn`](ErrorKind::Spawn),
    /// [`Cancelled`](ErrorKind::Cancelled), [`Parse`](ErrorKind::Parse),
    /// [`Io`](ErrorKind::Io)) — so a caller can fall back to the
    /// [`Display`](std::fmt::Display) message. For the raw, untrimmed stream
    /// match on [`kind`](Self::kind)'s fields directly.
    pub fn diagnostic(&self) -> Option<&str> {
        self.kind.diagnostic()
    }

    /// The captured standard output, for the kinds that carry a stream — a
    /// non-zero [`Exit`](ErrorKind::Exit), a [`Timeout`](ErrorKind::Timeout)
    /// (partial output before the kill), or a [`Signalled`](ErrorKind::Signalled)
    /// crash — `None` for every other kind. The raw stream in full (untrimmed);
    /// for the best one-line message use [`diagnostic`](Self::diagnostic), for
    /// both streams joined [`combined`](Self::combined).
    pub fn stdout(&self) -> Option<&str> {
        self.kind.stdout()
    }

    /// The captured standard error, for the stream-bearing kinds (see
    ...
    /// stream-bearing kind ([`Exit`](ErrorKind::Exit) /
    /// [`Timeout`](ErrorKind::Timeout) / [`Signalled`](ErrorKind::Signalled))
    /// produced by a checking verb built over
    /// [`output_bytes`](crate::Command::output_bytes) (e.g.
    /// `output_bytes().await?.ensure_success()?`); `None` for every other
    /// kind, and for a stream-bearing kind produced on the text path
    ...
    /// stream-bearing kinds, `None` otherwise. A `\n` is inserted between the
    ...
    /// kind that names one (all except [`Unsupported`](ErrorKind::Unsupported),
    /// [`Io`](ErrorKind::Io), and the `limits`-only `ResourceLimit`), `None` otherwise.
    /// The [`Error`] twin of
    /// [`ProcessResult::program`](crate::ProcessResult::program): the one
    /// cross-cutting datum a wrapper routes or logs on.
    ...
    ///
    /// `false` for every other kind — notably it does **not** fire for a
    /// missing or invalid working directory (a [`Spawn`](ErrorKind::Spawn) carrying
    ...
    /// [`Spawn`](ErrorKind::Spawn) / [`Io`](ErrorKind::Io) carrying
    /// [`PermissionDenied`](std::io::ErrorKind::PermissionDenied); `false`
    /// otherwise.
    pub fn is_permission_denied(&self) -> bool {
        self.kind.is_permission_denied()
    }
    ...
    /// violation. Classifies the [`Spawn`](ErrorKind::Spawn)/[`Io`](ErrorKind::Io) IO
    /// error only.
    ///
    /// **Scope: IO/spawn-level, never exit codes.** Whether a tool's non-zero
    /// [`Exit`](ErrorKind::Exit) is retryable is domain-specific (a `git` 128 is not
    /// generically transient) — that stays the caller's call. [`Timeout`](ErrorKind::Timeout)
    ...
    /// The process exit code for a non-zero [`Exit`](ErrorKind::Exit); `None` for
    /// every other kind (a timeout or a signal kill carries no exit code).
    ...
    /// `stats` feature), so a code is one name everywhere.
    pub fn code(&self) -> Option<i32> {
        self.kind.code()
    }

    /// The signal number for a [`Signalled`](ErrorKind::Signalled) run terminated
    /// with a **known** signal (**Unix only**); `None` for every other kind and
    /// for a signal the kernel didn't expose — the [`Error`] twin of
    /// [`ProcessResult::signal`](crate::ProcessResult::signal) /
    /// [`Outcome::signal`](crate::Outcome::signal).
    pub fn signal(&self) -> Option<i32> {
        self.kind.signal()
    }

    /// Which limit a [`ResourceLimit`](ErrorKind::ResourceLimit) failure is about;
    /// `None` for every other kind.
    #[cfg(feature = "limits")]
    pub fn limit_kind(&self) -> Option<crate::limits::LimitKind> {
        self.kind.limit_kind()
    }

    /// Why a [`ResourceLimit`](ErrorKind::ResourceLimit) failure occurred; `None`
    /// for every other kind.
    #[cfg(feature = "limits")]
    pub fn limit_reason(&self) -> Option<crate::limits::LimitReason> {
        self.kind.limit_reason()
    }

    /// Whether the run was killed because it exceeded its
    /// [`Command::timeout`](crate::Command::timeout) — i.e. this is a
    /// [`Timeout`](ErrorKind::Timeout). First-class here so the
    /// [`is_transient`](Self::is_transient) retry-composition example can read
    /// `e.is_transient() || e.is_timeout()` rather than matching the kind by hand.
    ...
    /// [`Cancelled`](ErrorKind::Cancelled). A caller that initiated the stop can
    /// swallow it rather than log or retry it as a real failure (the same
    /// disposition [`Supervisor`](crate::Supervisor) treats as terminal).
    pub fn is_cancelled(&self) -> bool {
        self.kind.is_cancelled()
    }

    /// Whether the run was killed by a signal — i.e. this is a
    /// [`Signalled`](ErrorKind::Signalled). Distinct from [`signal`](Self::signal):
    ...
    pub fn is_signalled(&self) -> bool {
        self.kind.is_signalled()
    }

    /// Construct an [`Exit`](ErrorKind::Exit) — a `#[doc(hidden)]` convenience for
    /// custom [`ProcessRunner`](crate::ProcessRunner) doubles and error-classifier
    /// tests, so they stop spelling out the struct literal (which the kind's
    ...
    /// Construct a [`Timeout`](ErrorKind::Timeout) — see [`exit`](Self::exit).
    #[doc(hidden)]
    pub fn timeout(
        program: impl Into<String>,
        timeout: Duration,
    ...
    /// Construct a [`Signalled`](ErrorKind::Signalled) — see [`exit`](Self::exit).
    #[doc(hidden)]
    pub fn signalled(
        program: impl Into<String>,
        signal: Option<i32>,
    ...
    /// Construct a [`Spawn`](ErrorKind::Spawn) — see [`exit`](Self::exit).
    #[doc(hidden)]
    ...
    /// Construct a [`NotFound`](ErrorKind::NotFound) — see [`exit`](Self::exit).
    #[doc(hidden)]
    ...
    /// Construct a [`Stdin`](ErrorKind::Stdin) — see [`exit`](Self::exit).
    #[doc(hidden)]
    ...
    /// Construct a [`Parse`](ErrorKind::Parse) from a caller-supplied parser's own
    ...
    /// `ErrorKind::Parse` once the kind is `#[non_exhaustive]`, and that path is
...
/// Wraps a bare [`ErrorKind`] into the boxed [`Error`] every fallible path in
/// the crate returns — the conversion every construction site (and the `?`
/// operator, where a helper returns `Result<T, ErrorKind>`) goes through.
...
/// Delegates to the boxed [`ErrorKind`]'s `Display` (derived by `thiserror`).
...
/// Delegates to the boxed [`ErrorKind`]'s `source()` (derived by `thiserror`
/// from each kind's `#[source]` field / `#[error(transparent)]`).
...
/// Delegates to the boxed [`ErrorKind`]'s manual `Debug` — see there for why
/// it isn't derived.
...
impl ErrorKind {
    ...
    fn diagnostic(&self) -> Option<&str> {
        ...
        match self {
            ErrorKind::Exit { stdout, stderr, .. }
            | ErrorKind::Timeout { stdout, stderr, .. }
            | ErrorKind::Signalled { stdout, stderr, .. } => exit_diagnostic(stdout, stderr),
            ErrorKind::Spawn { .. }
            | ErrorKind::NotFound { .. }
    ...
    fn stdout_bytes(&self) -> Option<&[u8]> {
        ...
        match self {
            ErrorKind::Exit { stdout_bytes, .. }
            | ErrorKind::Timeout { stdout_bytes, .. }
            | ErrorKind::Signalled { stdout_bytes, .. } => stdout_bytes.as_deref(),
            ErrorKind::Spawn { .. }
            | ErrorKind::NotFound { .. }
            | ErrorKind::CassetteMiss { .. }
            | ErrorKind::OutputTooLarge { .. }
    ...
    /// ([`Exit`](Self::Exit) / [`Timeout`](Self::Timeout) /
    /// [`Signalled`](Self::Signalled)), `None` for the rest — the single
    /// exhaustive match the stream accessors above derive from.
    fn streams(&self) -> Option<(&str, &str)> {
        ...
        match self {
            ErrorKind::Exit { stdout, stderr, .. }
            | ErrorKind::Timeout { stdout, stderr, .. }
            | ErrorKind::Signalled { stdout, stderr, .. } => Some((stdout, stderr)),
            ErrorKind::Spawn { .. }
            | ErrorKind::NotFound { .. }
            | ErrorKind::CassetteMiss { .. }
            | ErrorKind::OutputTooLarge { .. }
    ...
    fn program(&self) -> Option<&str> {
        ...
        match self {
            ErrorKind::Spawn { program, .. }
            | ErrorKind::NotFound { program, .. }
            | ErrorKind::CassetteMiss { program }
            | ErrorKind::Exit { program, .. }
            | ErrorKind::Timeout { program, .. }
            | ErrorKind::OutputTooLarge { program, .. }
            | ErrorKind::NotReady { program, .. }
            | ErrorKind::Parse { program, .. }
            | ErrorKind::Cancelled { program }
            | ErrorKind::Signalled { program, .. }
            | ErrorKind::Stdin { program, .. } => Some(program),
            ErrorKind::Unsupported { .. } | ErrorKind::Io(_) => None,
            #[cfg(feature = "limits")]
            ErrorKind::ResourceLimit { .. } => None,
        }
    }

    /// See [`Error::is_not_found`].
    fn is_not_found(&self) -> bool {
        matches!(self, ErrorKind::NotFound { .. })
    }

    /// See [`Error::is_permission_denied`].
    ...
    fn signal(&self) -> Option<i32> {
        ...
        match self {
            ErrorKind::Signalled { signal, .. } => *signal,
            ErrorKind::Spawn { .. }
            | ErrorKind::NotFound { .. }
            | ErrorKind::CassetteMiss { .. }
            | ErrorKind::Exit { .. }
    ...
    /// See [`Error::limit_kind`].
    #[cfg(feature = "limits")]
    fn limit_kind(&self) -> Option<crate::limits::LimitKind> {
        // Exhaustive on purpose (like `signal`/`program`): a future variant
        // must add itself here, not fall through a `_`.
        match self {
            ErrorKind::ResourceLimit { kind, .. } => Some(*kind),
            ErrorKind::Spawn { .. }
            | ErrorKind::NotFound { .. }
            | ErrorKind::CassetteMiss { .. }
            | ErrorKind::Exit { .. }
    ...
    /// See [`Error::limit_reason`].
    #[cfg(feature = "limits")]
    fn limit_reason(&self) -> Option<crate::limits::LimitReason> {
        // Exhaustive on purpose (like `signal`/`program`): a future variant
        // must add itself here, not fall through a `_`.
        match self {
            ErrorKind::ResourceLimit { reason, .. } => Some(*reason),
            ErrorKind::Spawn { .. }
            | ErrorKind::NotFound { .. }
            | ErrorKind::CassetteMiss { .. }
            | ErrorKind::Exit { .. }
    ...
    /// See [`Error::is_signalled`].
    fn is_signalled(&self) -> bool {
        matches!(self, ErrorKind::Signalled { .. })
    }

    /// The underlying [`std::io::Error`] for the variants that carry one
    /// ([`Spawn`](Self::Spawn), [`Io`](Self::Io)) — the basis for the io-level
    /// classifiers above.
    fn io_source(&self) -> Option<&std::io::Error> {
        ...
        match self {
            ErrorKind::Spawn { source, .. } => Some(source),
...
/// Manual `Debug`: bounds the [`Exit`](ErrorKind::Exit) streams and redacts
...
impl fmt::Debug for ErrorKind {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ErrorKind::Spawn { program, source } => f
                .debug_struct("Spawn")
                .field("program", program)
                .field("source", source)
                .finish(),
            ErrorKind::NotFound { program, searched } => f
                ...
                .finish(),
            ErrorKind::CassetteMiss { program } => f
            ...
            ErrorKind::OutputTooLarge {
                ...
                .finish(),
            ErrorKind::NotReady { program, timeout } => f
                .debug_struct("NotReady")
                .field("program", program)
                .field("timeout", timeout)
                .finish(),
            ErrorKind::Parse { program, message } => f
                .debug_struct("Parse")
                .field("program", program)
            ...
            ErrorKind::ResourceLimit {
                ...
                .finish(),
            ErrorKind::Unsupported { operation } => f
                .debug_struct("Unsupported")
                .field("operation", operation)
                .finish(),
            ErrorKind::Cancelled { program } => f
            ...
            ErrorKind::Signalled {
                ...
                .field("stdout_bytes", &BytesPreview(stdout_bytes.as_deref()))
                .finish(),
            ErrorKind::Stdin { program, source } => f
                .debug_struct("Stdin")
                .field("program", program)
                .field("source", source)
                .finish(),
            ErrorKind::Io(source) => f.debug_tuple("Io").field(source).finish(),
        }
    }
}

/// `Debug` for a captured stream, bounded to a 200-byte char-boundary preview
/// with a `(+N bytes)` note — the [`Exit`](ErrorKind::Exit) streams can be
/// multi-MiB and must never flood a `{e:?}` log line or `.unwrap()` panic
/// message. Mirrors the [`Display`](std::fmt::Display) tail cap in
/// [`display_exit`].
...
/// `Debug` for the `stdout_bytes` field of [`Exit`](ErrorKind::Exit) /
/// [`Timeout`](ErrorKind::Timeout) / [`Signalled`](ErrorKind::Signalled): never
/// dumps the raw bytes (they may be binary, may carry secrets, and can be
...
/// `Debug` for [`NotFound`](ErrorKind::NotFound)'s `searched`: the `PATH` value is an
...
/// Builds the [`ErrorKind::Io`] raised when a capture verb is called on a command
...
/// `detail` is bounded/sanitized like [`Parse`](ErrorKind::Parse)'s `message` — it
...
/// and the [`Parse`](ErrorKind::Parse) message head ([`display_parse`]) — so the
...
mod tests {
    ...
    fn stream_accessors_cover_the_stream_bearing_variants() {
        ...
        let not_ready = ErrorKind::NotReady {
            program: "server".into(),
    ...
    fn disposition_accessors_code_signal_timeout_cancelled() {
        ...
        let cancelled = ErrorKind::Cancelled {
            program: "job".into(),
        };
    ...
    fn program_accessor_covers_named_variants_and_skips_the_rest() {
        ...
        assert_eq!(
            ErrorKind::Io(std::io::Error::from(std::io::ErrorKind::PermissionDenied)).program(),
            None
        );
        #[cfg(feature = "limits")]
        assert_eq!(
    ...
    fn doc_hidden_constructors_build_the_expected_variants() {
        ...
        match Error::signalled("git", None, "o", "e").kind() {
            ErrorKind::Signalled { signal, .. } => assert_eq!(*signal, None),
    ...
    fn diagnostic_is_none_for_non_exit_variants() {
        ...
        let unsupported = ErrorKind::Unsupported {
            operation: "suspend".into(),
        };
        ...
        #[cfg(feature = "limits")]
        {
            let limit = ErrorKind::ResourceLimit {
                kind: crate::limits::LimitKind::Memory,
    ...
    fn timeout_and_signalled_carry_diagnostic_streams() {
        ...
        let signalled = ErrorKind::Signalled {
            program: "worker".into(),
            signal: Some(11),
    ...
    fn resource_limit_display_carries_kind_and_reason() {
        ...
        let unsupported = ErrorKind::ResourceLimit {
        ...
        let unenforceable = ErrorKind::ResourceLimit {
            kind: LimitKind::Cpu,
        ...
        let invalid = ErrorKind::ResourceLimit {
        ...
        let no_detail = ErrorKind::ResourceLimit {
            kind: LimitKind::Memory,
            reason: LimitReason::Unsupported,
            detail: String::new(),
        };
    ...
    fn resource_limit_accessors_read_kind_and_reason_without_destructuring() {
        ...
        let err = ErrorKind::ResourceLimit {
            kind: LimitKind::Cpu,
            reason: LimitReason::Invalid,
            detail: "boom".into(),
        };
    ...
    fn signalled_display_and_diagnostic() {
        let with_signal = ErrorKind::Signalled {
            program: "git".into(),
            signal: Some(9),
            stdout: String::new(),
            stderr: String::new(),
        ...
        let no_signal = ErrorKind::Signalled {
            program: "git".into(),
            signal: None,
            stdout: String::new(),
            stderr: String::new(),
    ...
    fn not_found_and_permission_denied_are_classified_on_spawn_and_io() {
        ...
        assert!(!ErrorKind::Io(std::io::Error::from(NotFound)).is_not_found());
    ...
    fn spawn_not_found_stdin_and_parse_constructors_build_the_expected_variant() {
        ...
        match stdin.kind() {
            ErrorKind::Stdin { program, source } => {
                assert_eq!(program, "git");
                assert_eq!(source.raw_os_error(), Some(32));
            }
            other => panic!("expected ErrorKind::Stdin, got {other:?}"),
        }

        let parse = Error::parse("git", "unexpected token");
        assert!(matches!(
            parse.kind(),
            ErrorKind::Parse { program, message }
