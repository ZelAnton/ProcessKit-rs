...
//!   path resolves to [`ErrorKind::Cancelled`]. Spawn-time sandboxing knobs:
...
//!   [`ProcessGroupOptions`], and `ErrorKind::ResourceLimit`. Implies `stats`.
...
/// # Errors
///
/// The same surface as [`Command::run`]: a launch failure ([`ErrorKind::NotFound`] /
/// [`ErrorKind::Spawn`] / [`ErrorKind::Unsupported`] / [`ErrorKind::Io`]), a non-accepted
/// exit ([`ErrorKind::Exit`]), [`ErrorKind::Signalled`], [`ErrorKind::Timeout`], or
/// [`ErrorKind::OutputTooLarge`] on a fail-loud truncation.
...
/// raised; beyond a launch failure, only [`ErrorKind::Cancelled`],
/// [`ErrorKind::OutputTooLarge`], [`ErrorKind::Stdin`], and [`ErrorKind::Io`] surface.
...
/// exact [`ErrorKind::NotFound`] a run would raise on Unix, where `execvp` searches
...
/// # Errors
///
/// [`ErrorKind::NotFound`] when the program can't be located
...
///
/// An empty `processes` slice is an error ([`ErrorKind::Io`] with
...
/// `ErrorKind::Cancelled` for a cancelled run, or [`ErrorKind::Stdin`] when its stdin
...
/// # Errors
///
/// [`ErrorKind::Io`] with [`InvalidInput`](std::io::ErrorKind::InvalidInput) when
/// `processes` is empty. Otherwise the first finisher's error surfaces:
/// [`ErrorKind::Cancelled`] (a cancelled run), [`ErrorKind::Stdin`] (a non-broken-pipe
/// stdin-source failure on an otherwise-successful exit), or [`ErrorKind::Io`] (a
...
/// `ErrorKind::Cancelled` (cancelled run) or [`ErrorKind::Stdin`] (a non-broken-pipe
...
/// # Errors
///
/// A contender's [`ErrorKind::Io`] (a failed reap), [`ErrorKind::Cancelled`] (a
/// cancelled run), or [`ErrorKind::Stdin`] (a non-broken-pipe stdin-source failure
...
mod tests {
    ...
    async fn wait_any_genuine_cancel_stays_cancelled_on_re_wait() {
        ...
        assert!(
            matches!(err2.kind(), crate::ErrorKind::Cancelled { .. }),
    ...
    async fn second_stdout_lines_errors_and_first_overflow_is_preserved() {
        ...
        assert!(
            matches!(err.kind(), crate::ErrorKind::OutputTooLarge { .. }),
