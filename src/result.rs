...
pub struct ProcessResult<T> {
    ...
    stderr: String,
    outcome: Outcome,
    /// Carried so the success-checking helpers can build a faithful [`ErrorKind::Timeout`].
    ...
    /// [`ErrorKind::OutputTooLarge`]; meaningful only when `truncated`.
    total_lines: usize,
    total_bytes: usize,
...
impl<T> ProcessResult<T> {
    ...
    /// # Errors
    ///
    /// - [`ErrorKind::Timeout`] if the run was killed by its deadline (checked
    ///   *first*, so a run that both timed out and exited non-zero reports the
    ///   timeout).
    /// - [`ErrorKind::Signalled`](crate::ErrorKind::Signalled) if it was terminated by a
        ...
        T: StdoutText,
    {
        match self.outcome {
            ...
            Outcome::TimedOut => Err(ErrorKind::Timeout {
            ...
            .into()),
            Outcome::Signalled(signal) => Err(ErrorKind::Signalled {
                program: self.program.clone(),
                signal,
                stdout: self.stdout.as_text(),
                stderr: self.stderr.clone(),
            ...
            .into()),
            Outcome::Exited(code) => Err(ErrorKind::Exit {
                program: self.program.clone(),
                code,
                stdout: self.stdout.as_text(),
                stderr: self.stderr.clone(),
    ...
    /// a timeout surfaces as [`ErrorKind::Timeout`], a signal-kill (no code) as
    /// [`ErrorKind::Signalled`], otherwise the code. Takes `self` by value (rather
    ...
    /// so a checking verb can build a faithful [`ErrorKind::OutputTooLarge`] when it
    ...
    /// drop-policy that discarded lines surfaces as [`ErrorKind::OutputTooLarge`]
    ...
    pub(crate) fn reject_if_truncated(
        ...
        if self.truncated {
            return Err(ErrorKind::OutputTooLarge {
                program: self.program.clone(),
                max_lines,
                max_bytes,
...
/// Render captured stdout as text for [`ErrorKind::Exit`], whatever the payload type:
...
mod tests {
    ...
    fn signalled_carries_signal_number() {
        ...
        match killed.ensure_success().unwrap_err().kind() {
    ...
    fn diagnostic_prefers_stderr_then_falls_back_to_stdout() {
        ...
        let ErrorKind::Exit { .. } = conflict.clone().ensure_success().unwrap_err().kind() else {
            panic!("expected Exit");
        };
        assert_eq!(
    ...
    fn timed_out_takes_precedence_over_exit_code() {
        ...
        match timed.ensure_success().unwrap_err().kind() {
            ErrorKind::Timeout {
                program,
                timeout,
                stdout,
    ...
    fn signal_kill_surfaces_as_signalled_error() {
        ...
        // `ErrorKind::Signalled` from both `require_code` and `ensure_success`,
        // never a synthetic `ErrorKind::Exit { code: -1 }` sentinel.
        let killed = ProcessResult::new(
            "git".into(),
            "out".to_owned(),
            String::new(),
        ...
        assert!(!killed.is_success());
        assert!(matches!(
            killed.clone().require_code().unwrap_err().kind(),
            ErrorKind::Signalled { .. }
        ));
        match killed.ensure_success().unwrap_err().kind() {
            ErrorKind::Signalled {
    ...
    fn error_exit_carries_full_streams() {
        ...
        let err = bad.ensure_success().unwrap_err();
        let ErrorKind::Exit { stdout, stderr, .. } = err.kind() else {
            panic!("expected Exit");
        };
    ...
    fn ok_codes_widen_success_without_masking_other_failures() {
        ...
        assert!(matches!(
            two.ensure_success().unwrap_err().kind(),
