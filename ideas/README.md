...
- `next-vcs-toolkit-feedback.md` — downstream feedback from the `vcs-toolkit-rs`
  multi-CLI wrapper: an `Error` accessor family (`stdout`/`stderr`/`combined`/
  `exit_code`/`is_timeout`), `Invocation` env-assertion helpers + test error
  constructors, `ProcessResult::output_contains_any`; plus concrete-consumer
  reinforcement of `later-retry-jitter` / `later-extensibility-hooks`.
