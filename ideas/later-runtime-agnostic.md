...
| **`async-process` as the runtime-agnostic backend** | **Just another hard runtime** | `async-process` brings its **own** reactor (`async-io`/`polling`). "Agnostic" resolves to depending on async-io's stack instead of tokio's — the same coupling one layer down, now behind a feature flag with double the test matrix. |
