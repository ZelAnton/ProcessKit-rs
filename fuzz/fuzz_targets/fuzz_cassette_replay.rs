...
fuzz_target!(|input: Input| {
    ...
    let cassette_entries: Vec<_> = entries.iter().map(|entry| {
        ...
        match entry.kind % 4 {
            0 => { object.insert("error".to_owned(), json!({ "kind": "Unsupported", "operation": entry.error_message })); }
            1 => { object.insert("code".to_owned(), json!(0)); }
            2 => { object.insert("timed_out".to_owned(), json!(true)); }
            _ => { object.insert("signal".to_owned(), json!(9)); }
        }
        value
    }).collect();
    let cassette = json!({ "version": 4, "entries": cassette_entries }).to_string();
    let calls: Vec<_> = input.calls.into_iter().take(64).enumerate().map(|(index, selector)| {
        if !entries.is_empty() && selector % 2 == 0 {
            ...
            while entries.iter().any(|entry| entry.program == program && entry.args == args) { args.push("_".to_owned()); }
            (program, args)
        }
    }).collect();
